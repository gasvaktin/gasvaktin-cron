...
def tweet_about_trends_changes():
    ...
    for company_key in company_keys:
        ...
        if timestamp_ten_minutes_ago < last_change['timestamp']:
            ...
            # check bensin
            bensin_price = 'median_bensin95'
            if (last_change[bensin_price] != before_change[bensin_price]):
                b_last_price = last_change[bensin_price]
                b_before_price = before_change[bensin_price]
                bensin_diff = round(b_last_price - b_before_price, 1)
            # check diesel
            diesel_price = 'median_diesel'
            if last_change[diesel_price] != before_change[diesel_price]:
                d_last_price = last_change[diesel_price]
                d_before_price = before_change[diesel_price]
        ...
        if tweet_msg is not None:
            print 'tweet_message, (%s)' % (len(tweet_msg), )
            print '---'
            print tweet_msg
            print '---'
